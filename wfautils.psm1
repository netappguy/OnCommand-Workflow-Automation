...
.DESCRIPTION
Let log entry in WFA Log event and in a given file.
...
.EXAMPLE
Add-LogInfo -LogPath $scriptlog -Severity "Info" -MsgLine $("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~| " + $VersionInfo + " |~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
...
param (
	...
	{
		[string]$NewLogMessage = Get-date -format yyyyMMddHHmmss
		$NewLogMessage += " " + $Severity + " " + $(Get-WfaRestParameter "commandName") + " " + $MsgLine + "`n"
		Add-Content -Value $NewLogMessage -Path $LogPath
	}
	switch ($Severity)
	{
		"Info" { Get-WFALogger -Info -message $($MsgLine) }
		"Warn" { Get-WFALogger -Warn -message $($MsgLine) }
		"Error" { Get-WFALogger -Error -message $($MsgLine) }
		default { Get-WFALogger -Info -message $($MsgLine)}
	}
}
...
Cmdlet to Create a File log and the path (if needed) for it. The name and path are then returned.
...
# 	Get-WFAInstallDir
#
#	In this commandlet we get the WFA Installation directory, like "C:\Program Files\NetApp\WFA\"
#
#########################################################################################################################
...
# 	Get-WFABDRootPWD
#
#	In this commandlet we get the root Database password, like "EGBSFFD"
#
#########################################################################################################################
...
export-modulemember -function Add-LogInfo
